...
const Home: NextPage = (props: any) => {
  ...
  const typed = useTyped([
    ...
    t('home:typed.1.blockchain'),
    t('home:typed.2.web'),
    t('home:typed.3.restfull'),
  ...
  return (
    <Fragment>
      <Head>
        <title>Marvin Washington&mdash; Full-Stack Developer</title>
      </Head>
      ...
      <section className={styles.home}>
        <header className={styles.homeHeader}>
          ...
          <div className={styles.homeHeaderContent}>
            <h1 className={styles.homeHeaderContentHeading}>
              {t('home:headers.0.heading')}
            </h1>
            ...
            <Link href='/'>
              <div className={styles.homeHeaderContentButton}>
                {t('common:explore')}
              </div>
            </Link>
          </div>
          ...
          <div className={styles.homeHeaderLogs}>
            <div className={styles.homeHeaderLogsBox}>
              ...
              <p className={styles.homeHeaderLogsBoxParagraph}>
                {t('home:headers.1.customers')}
              </p>
            </div>
            <div className={styles.homeHeaderLogsBox}>
              <span className={styles.homeHeaderLogsBoxNumber}>60+</span>
              <p className={styles.homeHeaderLogsBoxParagraph}>
                {t('home:headers.2.projects')}
              </p>
            </div>
            <div className={styles.homeHeaderLogsBox}>
              <span className={styles.homeHeaderLogsBoxNumber}>7+</span>
              <p className={styles.homeHeaderLogsBoxParagraph}>
                {t('home:headers.3.experience')}
              </p>
            </div>
            <div className={styles.homeHeaderLogsBox}>
              <span className={styles.homeHeaderLogsBoxNumber}>5+</span>
              <p className={styles.homeHeaderLogsBoxParagraph}>
                {t('home:headers.4.honors')}
              </p>
            </div>
          </div>
        </header>

        <h4 className={styles.homeHeadingSecondary}>
          {t('home:about.0.heading')}
        </h4>
        <div className={styles.homeAbout}>
          <h2 className={styles.homeAboutParagraph}>
            {t('home:about.1.paragraph')}
          </h2>
                ...
                {home.fats.frontend.map((front) => {
                  return (
                    <Tooltip key={front.id} content={front.name}>
                      <li
                        key={front.id}
                        className={styles.homeAboutFATItemListLogoItem}
                      >
                        <span>
                          <Image
                            src={front.src}
        ...
        <h4 className={styles.homeHeadingPrimary}>
          {t('home:services.6.heading')}
        </h4>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const repositories = await axios.get(
    'https://api.github.com/users/perseus8/repos'
  );
  const organizations = await axios.get(
    'https://api.github.com/users/perseus8/orgs'
  );

  if (!repositories.data || !organizations.data) return { notFound: true };
